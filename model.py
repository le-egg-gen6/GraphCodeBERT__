...
class RoBERTaClassificationHead(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prop)
...
class Model(nn.Module):
    ...
    def forward(self, inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2, position_idx_2, attn_mask_2, labels=None):
        ...
        inputs_ids = torch.cat((inputs_ids_1.unsqueze(1), inputs_ids_2.unsqueeze(1)), 1).view(bs * 2, l)
        ...
        outputs = self.encoder.roberta(input_embeds=inputs_embeddings, attention_mask=attn_mask, position_ids=position_idx, token_type_ids=position_idx.eq(-1).long())[0]
